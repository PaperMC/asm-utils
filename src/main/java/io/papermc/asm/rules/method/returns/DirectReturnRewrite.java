...
public record DirectReturnRewrite(Set<ClassDesc> owners, ClassDesc existingType, TargetedMethodMatcher targetedMethodMatcher, Method staticHandler, boolean includeOwnerContext) implements TargetedTypeGeneratedStaticRewrite.Return, OwnableMethodRewriteRule.Filtered {
    ...
    public DirectReturnRewrite {
        ...
        if (!desc(staticHandler.getReturnType()).equals(targetedMethodMatcher.targetType())) {
        ...
        if (staticHandler.getParameterCount() != expectedStaticHandlerParamCount) {
            throw new IllegalArgumentException("staticHandler should only have %s parameter(s) of type %s".formatted(expectedStaticHandlerParamCount, (includeOwnerContext ? owner + " and " : "") + targetedMethodMatcher.targetType()));
        }
